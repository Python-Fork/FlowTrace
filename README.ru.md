...
FlowTrace поддерживает контекстный менеджер для удобного включения трассировки внутри ограниченного блока кода.  
Это полезно при написании тестов, анализе отдельных функций или работе с асинхронными задачами.
...
print_tree()
```
Каждый блок ```with active_tracing()```: создаёт собственную изолированную сессию трассировки,
поэтому параллельные или вложенные запуски не влияют друг на друга.
---
